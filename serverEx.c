...
void handleCommand(int fd, fd_set* allsocket, int* max_socket_so_far, User* userList){
    ...
    if((bytes_read = recv(fd, buffer, BUFFER_SIZE - 1, 0)) < 0) {
        ...
        if (strncmp("USER", buffer, 4) == 0) {
            ...
            cwdCom(userList, index, fd, buffer);
        }
...
void cwdCom(User* userList, int index, int fd, char* buffer){
    if (!userList[index].auth) {
        send_msg(fd, "530 Not logged in.\n");
        return;
    }
    ...
    char* path = buffer + 3; // Skip "CWD"

    // Skip whitespace
    while (*path == ' ' || *path == '\t') path++;

    // Trim trailing whitespace and control characters
    char *newline = strpbrk(path, "\r\n");
    if (newline) *newline = '\0';

    // Check if path is empty
    if (strlen(path) == 0) {
        send_msg(fd, "550 Failed to change directory.\n");
        return;
    }

    // Build upon the existing directory by appending the new path
    char tempDir[PATH_MAX];
    if (strcmp(userList[index].dir, "/") == 0) {
        snprintf(tempDir, PATH_MAX, "/%s", path);
    } else {
        snprintf(tempDir, PATH_MAX, "%s/%s", userList[index].dir, path);
    }

    // Resolve "." and ".." in tempDir
    char resolvedDir[PATH_MAX];
    if (!resolvePath(tempDir, resolvedDir)) {
        send_msg(fd, "550 Failed to change directory.\n");
        return;
    }

    // Ensure that resolvedDir starts with '/'
    if (resolvedDir[0] != '/') {
        send_msg(fd, "550 Failed to change directory.\n");
        return;
    }
    ...
    snprintf(serverPath, PATH_MAX, "./%s%s", userList[index].username, resolvedDir);

    // Check if serverPath is a directory
    struct stat st;
    if (stat(serverPath, &st) != 0 || !S_ISDIR(st.st_mode)) {
        send_msg(fd, "550 Failed to change directory.\n");
        return;
    }

    // Update user's current directory
    strncpy(userList[index].dir, resolvedDir, sizeof(userList[index].dir));
...
int resolvePath(const char* path, char* resolvedPath) {
    // Initialize variables
    char tempPath[PATH_MAX];
    strncpy(tempPath, path, PATH_MAX);
    tempPath[PATH_MAX - 1] = '\0'; // Ensure null-termination

    // Initialize the stack for path components
    char *stack[PATH_MAX];
    int top = -1;
    ...
    // Special case: if no components, the path is "/"
    if (top < 0) {
        strcpy(result, "/");
    }

    // Copy the result to resolvedPath
    strncpy(resolvedPath, result, PATH_MAX);
    resolvedPath[PATH_MAX - 1] = '\0'; // Ensure null-termination
